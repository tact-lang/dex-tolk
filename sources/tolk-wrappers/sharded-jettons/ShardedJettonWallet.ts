...
export class ShardedJettonWallet implements Contract {
    constructor(
    ...
    static createFromAddress(address: Address) {
        return new ShardedJettonWallet(address)
    }

    static createFromConfig(config: JettonWalletConfig, code: Cell, workchain = 0) {
        const data = jettonWalletConfigToCell(config)
        const init = {code, data}
        return new ShardedJettonWallet(contractAddress(workchain, init), init)
    }
    ...
    async sendTransfer(
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: ShardedJettonWallet.transferMessage(
                jetton_amount,
                to,
                responseAddress,
                customPayload,
    ...
    */
    ...
    async sendBurn(
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: ShardedJettonWallet.burnMessage(jetton_amount, responseAddress, customPayload),
            value: value,
        })
    }
    /*
    ...
    */
    ...
    async sendWithdrawTons(provider: ContractProvider, via: Sender) {
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: ShardedJettonWallet.withdrawTonsMessage(),
            value: toNano("0.1"),
        })
    }
    /*
    ...
    */
    ...
    async sendWithdrawJettons(
        ...
        await provider.internal(via, {
            sendMode: SendMode.PAY_GAS_SEPARATELY,
            body: ShardedJettonWallet.withdrawJettonsMessage(from, amount),
            value: toNano("0.1"),
        })
    }
}
