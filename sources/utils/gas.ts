...
import type {Blockchain, BlockchainTransaction} from "@ton/sandbox"
import chalk from "chalk"
import Table from "cli-table3"

export function getUsedGasInternal(transactions: BlockchainTransaction[]): number {
    return transactions.map(t => getComputeGasForTx(t)).reduceRight((prev, cur) => prev + cur)
}
...
export function printBenchmarkTable(results: BenchmarkResult[], args: BenchmarkTableArgs): void {
    ...
    const codeSizeMetrics = Object.keys(results[0]!.size)

    const codeSizeTable = createTable(results, codeSizeMetrics, "size")
